...
class LDAReader(object):
	def __init__(self, datasets, max_sent=None, block_size=1):
		"""
			datasets: datasets
			max_sent: maximum number of sentences to be read in each dataset
			block_size: number of sentences to yield (default=1)
		"""
		self.datasets = datasets
		self.max_sent = max_sent if max_sent else float('inf')
		if self.max_sent < float('inf'):
...
def train_lda(args):
	print "[LDA > n_topics: %d ]"
	lda_reader = LDAReader(args.ds, max_sent=args.max_sent)	
	ldazito = LdaMulticore(lda_reader,id2word=lda_reader.idx2wrd,num_topics=100, workers=3)
...
def get_parser():
	...
	parser.add_argument('-dim', type=int, default=100, help='size of embeddings or number of topics')
...
if __name__ == "__main__":	
	...
	print "[input > %s | max_sent: %d | workers: %d | output@%s]\n" % (repr(args.ds), args.max_sent, args.workers,  args.out)	
